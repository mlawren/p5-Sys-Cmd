...
*{ $M . Object::new } = sub {
    ...
    my $s = bless {@_}, $c;
    my %n = %{ $c . :: . ':E' };
    map { $s->{$_} = $n{$_}->() if !exists $s->{$_} } keys %n;
    $s;
};
*{ $M . import } = sub {
    import warnings;
...
*{ $M . 'build::e' } = sub {
    ...
    $e->{new} = sub {
        $c = shift;
        my $s = &{ $M . Object::new }( $c, @_ );
        my @B;
        do { @B = ( $c . ::BUILD, @B ) } while ($c) = @{ $c . ::ISA };
        exists &$_ && &$_($s) for @B;
        $s;
      }
};
...
*{ $M . 'required::e' } = sub {
    ...
    $o->{required} = sub {
        my ( $m, $n, %a ) = @_;
        if ( $a{required} ) {
            my $C = *{ $P . "new" }{CODE} || *{ $M . Object::new }{CODE};
            no warnings 'redefine';
            *{ $P . "new" } = sub {
                my $s = $C->(@_);
                my %a = @_[ 1 .. $#_ ];
                die $n . " required" if !exists $a{$n};
                $s;
              }
        }
        $m;
...
*{ $M . 'default::e' } = sub {
    ...
    $o->{default} = sub {
        my ( $m, $n, %a ) = @_;
        exists $a{default} or return $m;
        my ( $d, $r ) = $a{default};
        my $g =
            'HASH' eq ( $r = ref $d ) ? sub { +{%$d} }
          : 'ARRAY' eq $r ? sub { [@$d] }
          : 'CODE' eq $r  ? $d
          :                 sub { $d };
        my $i = exists $a{lazy} ? $a{lazy} : !${ $P . ':N' };
        $i or ${ $P . ':E' }{$n} = $g and return $m;
        sub {
...
Sys::Cmd::Mo - Inlined Mo for Sys::Cmd

=head1 DESCRIPTION

See L<Mo> for details.
