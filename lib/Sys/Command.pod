...
B<Sys::Command> allows you to call run external (system) commands from
Perl. It is intended as a more powerful version of the C<system>
built-in, but at the same time be a simpler version of L<IPC::Open3>,
L<IPC::Run> and L<IPC::Cmd>.

B<Sys::Command> uses L<File::Which> internally to discover the location
of the binary you are trying to run.
...
=item run($cmd, [@args], [\%opts]) => $output

run() will simply run the $cmd and @args provided, returning the output
as a string (in scalar context) or as a list (in list-context). It will
raise an exception on error. An optional \%opts hashref may be given to
control how (directory, environment, input etc) the command will be
called.

=item runa($cmd, [@args], [\%opts]) => $out/errput

runa() is the same as run() except the command's error output is
appeneded to the normal output.

=item spawn($cmd, [@args], [\%opts]) => Sys::Command::Process

spawn() will run the $cmd and @args with \%opts and return a object
which can be manipulated to interact with the command. See
L<Sys::Command::Process> for details.
...
Git where you need to manually set the same environment or encoding
each time. You can instead create a B<Sys::Command> object which holds
those values as a kind of template. The B<Sys::Command> constructor
takes the same arguments as B<Sys::Command::Process> with the exception
of 'input':
...
L<Sys::Command::Process>, L<IPC::Open3>, L<IPC::Run>, L<IPC::Cmd>.
