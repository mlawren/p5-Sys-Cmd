...
=head1 SYNOPSIS
    ...
    my $cmd = Sys::Command::Process->new(@cmd);

    # options can be passed as a hashref
    $cmd = Sys::Command::Process->new( @cmd, \%options );

    # Interact with the command via its STDIN, STDERR etc.
    if ($cmd->pid) {
        while (my $line = $cmd->stdout) {
            $cmd->stdin->print("next input");
        }
    }
    my @errors = $cmd->stderr->getlines;
